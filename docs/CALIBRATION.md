...
Media loads lazily from the original host with no referrer. Recalibration uses the selected provider. Claude sends selected examples, explanations, and supported images to Anthropic using the user's key; image examples require Analyze images to remain enabled. On-device processing uses text only and never falls back to Claude. Requests are bounded, run sequentially, and stop on settings changes or model errors. A local context-limit error is reported without changing the rule. No API key is stored with a filter set or feedback record.
