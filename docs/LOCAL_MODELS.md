...
This experimental implementation needs real-model quality and latency validation before being recommended as a default.
...
- Text classification, consumption tags, rule suggestions, and feedback recalibration all honor the selected provider. Local mode needs no API key. Cloud mode remains the default for existing installations.
